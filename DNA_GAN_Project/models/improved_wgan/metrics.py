...
def calculate_kmer_diversity(sequences, k=3):
    """
    Calculate the k-mer diversity of sequences.
    ...
    for i in range(batch_size):
        seq = ''.join([nucleotides[idx] for idx in indices[i]])
    ...
    # Normalize by maximum entropy (log2 of number of possible k-mers)
    max_entropy = np.log2(min(4**k, batch_size * (seq_len - k + 1)))
...
def evaluate_model(generator, data_loader, noise_dim=100, device='cuda', num_samples=1000):
    ...
    metrics = {
        ...
        'generated_kmer_diversity': calculate_kmer_diversity(generated_sequences),
        'real_kmer_diversity': calculate_kmer_diversity(real_sequences)
    }
