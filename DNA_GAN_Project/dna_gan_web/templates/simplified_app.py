...
def load_fasta_sequences(file_path, max_sequences=1000):
    ...
    try:
        # First try: Use BioPython's SeqIO parser with fasta format
        try:
            ...
            st.warning(f"Standard FASTA parser failed: {str(e)}. Trying alternative formats...")

            # Second try: Use fasta-blast format (handles comments better)
            try:
                ...
                st.warning(f"FASTA-BLAST parser also failed: {str(e2)}. Trying manual parsing...")
        ...
        if sequences:
            st.success(f"Successfully loaded {len(sequences)} sequences from {file_path}")
        else:
            ...
            try:
                ...
                if matches:
                    ...
                    if sequences:
                        st.warning(f"Extracted {len(sequences)} DNA-like sequences using pattern matching")
                    else:
                        st.error(f"No valid sequences found in {file_path}")
                else:
                    st.error(f"No DNA-like patterns found in {file_path}")
            except Exception as e4:
                st.error(f"Failed to extract DNA-like patterns: {str(e4)}")

    except Exception as e:
        st.error(f"Unexpected error loading sequences from {file_path}: {str(e)}")
...
def plot_gc_content_distribution(gc_content, title):
    ...
    if len(gc_content_pct) > 5:  # Only add KDE if we have enough data points
        ...
        hist, bin_edges = np.histogram(gc_content_pct, bins=30)
    ...
    # Add mean line
    mean_gc = np.mean(gc_content_pct)
    fig.add_shape(
        type="line",
        x0=mean_gc,
        y0=0,
...
def generate_sequences(model_type, num_sequences=100, status_container=None):
    """
    ...
    """
    # Create a status container if not provided
    if status_container is None:
        status_container = st.empty()

    # Function to display status messages in the container
    ...
    try:
        ...
        if model_type == 'gumbel':
            ...
            local_model_script = "models/gumbel_softmax_gan/generate_sequences.py"
            simple_model_script = "models/gumbel_softmax_gan/generate_sequences_simple.py"
            gc_bias = 0.45  # Slightly lower GC content
        else:
            output_file = "data/improved_generated_sequences.fasta"
            model_name = "Improved WGAN-GP"
            local_model_script = "models/improved_wgan/generate_sequences.py"
            simple_model_script = "models/improved_wgan/generate_sequences_simple.py"
            gc_bias = 0.52  # Slightly higher GC content

        # Create output directory if it doesn't exist
        os.makedirs(os.path.dirname(output_file), exist_ok=True)

        # First, try to use the simplified model script (no PyTorch dependency)
        if os.path.exists(simple_model_script):
            show_status(f"Generating sequences using simplified {model_name} model...")

            # Use subprocess to run the simplified model script
            cmd = [
                "python",
                simple_model_script,
            ...
            if result.returncode == 0:
                show_status(f"Successfully generated {num_sequences} sequences using simplified {model_name} model!", "success")
                return
            else:
                show_status(f"Error running simplified model: {result.stderr}. Trying full model...", "warning")

        # Try to use the full GAN model to generate sequences
        try:
            # Try to import PyTorch to check if it's available
            import torch
            show_status(f"PyTorch is available. Attempting to use full {model_name} model...")

            # Try to import the module directly
            try:
                if model_type == 'gumbel':
                    # Use a direct import to avoid module path issues
                    sys.path.append(os.path.abspath("."))
                    from models.gumbel_softmax_gan.generate_sequences import generate_sequences as gen_func
                else:
                    sys.path.append(os.path.abspath("."))
                    from models.improved_wgan.generate_sequences import generate_sequences as gen_func

                # Generate sequences
                gen_func(num_sequences, output_file)
                show_status(f"Successfully generated {num_sequences} sequences using full {model_name} model!", "success")
                return
            except ImportError as e:
                show_status(f"Could not import model module: {str(e)}. Trying subprocess approach...", "warning")

                # Try using subprocess to run the script
                if os.path.exists(local_model_script):
                    # Use subprocess to run the model script with environment variables
                    cmd = [
                        "python",
                        local_model_script,
                        "--output_file", output_file,
                    ...
                    if result.returncode == 0:
                        show_status(f"Successfully generated {num_sequences} sequences using {model_name}!", "success")
                        return
                    else:
                        show_status(f"Error running model: {result.stderr}. Falling back to random sequence generation.", "warning")
                else:
                    show_status(f"Model script {local_model_script} not found. Falling back to random sequence generation.", "warning")
        except ImportError:
            show_status("PyTorch is not available. Falling back to random sequence generation.", "warning")

        # Fallback: Generate random sequences with appropriate GC bias
        show_status(f"Generating random sequences with {model_name} characteristics...")
        ...
        show_status(f"Generated {num_sequences} random sequences with {model_name} characteristics", "success")
        show_status("Sequences generated successfully!", "success")

    except Exception as e:
        show_status(f"Error generating sequences: {str(e)}", "error")
        # Print the full traceback for debugging
        import traceback
        show_status(f"Traceback: {traceback.format_exc()}", "error")

        # Still generate some sequences as a last resort
        try:
            # Create output directory if it doesn't exist
            os.makedirs(os.path.dirname(output_file), exist_ok=True)
            ...
            show_status(f"Generated {num_sequences} random sequences as a fallback after error", "warning")
        except Exception as e2:
            show_status(f"Failed to generate fallback sequences: {str(e2)}", "error")
...
def main():
    ...
    processed_data_path = "data/preprocessed_dna_sequences.fasta"

    # Define file paths
    if data_path_env:
        original_data_path = data_path_env
        st.sidebar.info(f"Using data path from environment variable: {original_data_path}")
    else:
        original_data_path = "D:/Experiential Learning/Biotech/data/seq_download.pl.fasta"

    # Process the original data if it exists
    if os.path.exists(original_data_path):
        st.sidebar.success(f"Original data file found at {original_data_path}")

        # Check if the file is an HTML file
        try:
            with open(original_data_path, 'r', encoding='utf-8', errors='ignore') as f:
                first_line = f.readline().strip()

            if first_line.startswith('<!') or first_line.startswith('<html'):
                st.sidebar.warning("Original data appears to be an HTML file. Extracting FASTA content...")

                # Extract FASTA content
                try:
                    from utils.extract_fasta_from_html import extract_and_validate_fasta

                    # Ensure the data directory exists
                    os.makedirs(os.path.dirname(processed_data_path), exist_ok=True)

                    # Extract FASTA content
                    success = extract_and_validate_fasta(original_data_path, processed_data_path)

                    if success:
                        st.sidebar.success(f"Extracted FASTA content to {processed_data_path}")
                        original_data_path = processed_data_path
                    else:
                        st.sidebar.error("Failed to extract FASTA content. Using local data if available.")
                        original_data_path = processed_data_path
                except Exception as e:
                    st.sidebar.error(f"Error extracting FASTA content: {str(e)}")
                    original_data_path = processed_data_path
        except Exception as e:
            st.sidebar.error(f"Error checking original data file: {str(e)}")
    else:
        st.sidebar.warning(f"Original data file not found at {original_data_path}. Using local data if available.")
        original_data_path = processed_data_path

    # Check if the processed data exists, if not, create a sample file
    if not os.path.exists(original_data_path):
        st.sidebar.warning(f"Local data file not found at {original_data_path}. Generating sample data.")
        ...
        try:
            ...
            st.sidebar.success(f"Generated sample data at {original_data_path}")
        except Exception as e:
    ...
    # Add a status indicator for data loading
    ...
    with status_container:
        ...
        <div class="status-box">
            ...
            <div class="status-grid">
                <div class="status-label">Original Data:</div>
                <div class="status-value" id="original-status">
                    {original_status}
                </div>
                <div class="status-label">Gumbel-Softmax GAN:</div>
                <div class="status-value" id="gumbel-status">
                    {gumbel_status}
                </div>
                <div class="status-label">Improved WGAN-GP:</div>
                <div class="status-value" id="improved-status">
                    {improved_status}
                </div>
            </div>
        </div>
        """.format(
            original_status="<span class='status-{}'>{}</span>".format(
                "success" if os.path.exists(original_data_path) else "error",
                "Available" if os.path.exists(original_data_path) else "Not Found"
            ),
            gumbel_status="<span class='status-{}'>{}</span>".format(
                "success" if os.path.exists(gumbel_data_path) else "warning",
                "Available" if os.path.exists(gumbel_data_path) else "Not Generated"
            ),
            improved_status="<span class='status-{}'>{}</span>".format(
                "success" if os.path.exists(improved_data_path) else "warning",
                "Available" if os.path.exists(improved_data_path) else "Not Generated"
            )
        ), unsafe_allow_html=True)
    ...
    st.sidebar.info(f"Original data: {original_data_path}")
    ...
    if page == "Home":
        ...
        with col1:
            ...
            if st.button("Generate Gumbel-Softmax Sequences"):
                # Create a status container for messages
                status_container = st.empty()
                with st.spinner("Generating sequences..."):
                    generate_sequences('gumbel', num_sequences=100, status_container=status_container)
        ...
        with col2:
            ...
            if st.button("Generate Improved WGAN-GP Sequences"):
                ...
                status_container = st.empty()
                with st.spinner("Generating sequences..."):
                    generate_sequences('improved', num_sequences=100, status_container=status_container)
    ...
    # Original Data page
        ...
        if not os.path.exists(original_data_path):
            ...
            if st.button("Generate Sample Original Sequences", key="gen_orig_btn", help="Generate sample DNA sequences to use as original data"):
                with st.spinner("Generating sample sequences..."):
                    try:
                        ...
                        st.error(f"Error generating sample sequences: {str(e)}")

        # Load the sequences
        original_sequences = load_fasta_sequences(original_data_path)

        if not original_sequences:
            st.warning("No valid sequences found in the original data file.")

            # Add a button to regenerate sample data
            if st.button("Generate New Sample Sequences", key="regen_orig_btn"):
                with st.spinner("Generating new sample sequences..."):
                    try:
                        ...
                        st.error(f"Error generating new sample sequences: {str(e)}")
    ...
    # Generated Data page
        ...
        if not os.path.exists(data_path):
            st.warning(f"{model_name} sequences have not been generated yet.")

            # Add a button to generate sequences with a custom key
            if st.button(f"Generate {model_name} Sequences", key=f"gen_{model_key}_btn",
                        help=f"Generate DNA sequences using the {model_name} model"):
                # Create a status container for messages
                status_container = st.empty()
                with st.spinner(f"Generating {model_name} sequences..."):
                    try:
                        generate_sequences(model_key, num_sequences=100, status_container=status_container)
                    except Exception as e:
                        with status_container:
                            st.error(f"Error generating {model_name} sequences: {str(e)}")
                            st.error("Using fallback method to generate sequences...")
                        try:
                            ...
                            with open(data_path, 'w') as f:
                                for i in range(100):
                                    seq = generate_random_dna_sequence(length=150, gc_bias=gc_bias)
                                    f.write(f">{model_key}_{i+1}\n")
                                    f.write(f"{seq}\n")

                            st.success(f"Generated fallback sequences for {model_name}")
                        except Exception as e2:
                            st.error(f"Fallback generation also failed: {str(e2)}")

        # Load the sequences
        sequences = load_fasta_sequences(data_path)

        if not sequences:
            st.error(f"No valid sequences found in the {model_name} data file.")

            # Add a button to regenerate sequences with a different key
            if st.button(f"Regenerate {model_name} Sequences", key=f"regen_{model_key}_btn"):
                # Create a status container for messages
                status_container = st.empty()
                with st.spinner(f"Regenerating {model_name} sequences..."):
                    try:
                        generate_sequences(model_key, num_sequences=100, status_container=status_container)
                        sequences = load_fasta_sequences(data_path)
                    except Exception as e:
                        with status_container:
                            st.error(f"Error regenerating {model_name} sequences: {str(e)}")
    ...
    # Model Comparison page
        ...
        # Status container for data availability
        status_container = st.container()
        with status_container:
            col1, col2, col3 = st.columns(3)
            ...
            with col1:
                if os.path.exists(original_data_path):
                    st.success("✅ Original Data Available")
                else:
                    st.error("❌ Original Data Missing")

            with col2:
                if os.path.exists(gumbel_data_path):
                    st.success("✅ Gumbel-Softmax Data Available")
                else:
                    st.warning("⚠️ Gumbel-Softmax Data Missing")

            with col3:
                if os.path.exists(improved_data_path):
                    st.success("✅ Improved WGAN-GP Data Available")
                else:
                    st.warning("⚠️ Improved WGAN-GP Data Missing")
        ...
        if st.button("Generate New Sequences for Both Models", key="gen_both_btn",
            ...
            with st.spinner("Generating sequences for both models..."):
                try:
                    ...
                    with status_container:
                        st.info("Generating Gumbel-Softmax GAN sequences...")
                    generate_sequences('gumbel', num_sequences=100, status_container=status_container)

                    # Generate Improved WGAN-GP sequences
                    with status_container:
                        st.info("Generating Improved WGAN-GP sequences...")
                    generate_sequences('improved', num_sequences=100, status_container=status_container)
                    ...
                    with status_container:
                        st.error(f"Error generating sequences: {str(e)}")
        ...
        if os.path.exists(original_data_path):
            with st.spinner("Loading original sequences..."):
                ...
                if original_sequences:
                    with status_messages:
                        st.success(f"Successfully loaded {len(original_sequences)} sequences from {original_data_path}")
                else:
                    st.error("No valid sequences found in the original data file.")

                # Generate original sequences if needed
                if st.button("Generate Sample Original Sequences", key="comp_orig_btn"):
                    with st.spinner("Generating sample sequences..."):
                        try:
                            from utils.generate_test_sequences import generate_test_sequences
                            generate_test_sequences(
            ...
            # Generate original sequences if needed
            if st.button("Generate Sample Original Sequences", key="comp_orig_miss_btn"):
                with st.spinner("Generating sample sequences..."):
                    try:
                        from utils.generate_test_sequences import generate_test_sequences
                        generate_test_sequences(
        ...
        if os.path.exists(gumbel_data_path):
            with st.spinner("Loading Gumbel-Softmax sequences..."):
                ...
                if gumbel_sequences:
                    with status_messages:
                        st.success(f"Successfully loaded {len(gumbel_sequences)} sequences from data/gumbel_generated_sequences.fasta")
                else:
                    st.warning("No valid sequences found in the Gumbel-Softmax data file.")

                # Generate Gumbel sequences if needed
                if st.button("Generate Gumbel-Softmax Sequences", key="comp_gumbel_btn"):
                    # Create a status container for messages
                    status_container = st.empty()
                    with st.spinner("Generating Gumbel-Softmax sequences..."):
                        try:
                            generate_sequences('gumbel', num_sequences=100, status_container=status_container)
                            gumbel_sequences = load_fasta_sequences(gumbel_data_path)
                        except Exception as e:
                            with status_container:
                                st.error(f"Error generating Gumbel-Softmax sequences: {str(e)}")
        else:
            st.warning("Gumbel-Softmax data file not found.")

            # Generate Gumbel sequences if needed
            if st.button("Generate Gumbel-Softmax Sequences", key="comp_gumbel_miss_btn"):
                # Create a status container for messages
                status_container = st.empty()
                with st.spinner("Generating Gumbel-Softmax sequences..."):
                    try:
        ...
        if os.path.exists(improved_data_path):
            with st.spinner("Loading Improved WGAN-GP sequences..."):
                ...
                if improved_sequences:
                    with status_messages:
                        st.success(f"Successfully loaded {len(improved_sequences)} sequences from data/improved_generated_sequences.fasta")
                else:
                    st.warning("No valid sequences found in the Improved WGAN-GP data file.")

                # Generate Improved sequences if needed
                if st.button("Generate Improved WGAN-GP Sequences", key="comp_improved_btn"):
                    # Create a status container for messages
                    status_container = st.empty()
                    with st.spinner("Generating Improved WGAN-GP sequences..."):
                        try:
                            generate_sequences('improved', num_sequences=100, status_container=status_container)
                            improved_sequences = load_fasta_sequences(improved_data_path)
                        except Exception as e:
                            with status_container:
                                st.error(f"Error generating Improved WGAN-GP sequences: {str(e)}")
        else:
            st.warning("Improved WGAN-GP data file not found.")

            # Generate Improved sequences if needed
            if st.button("Generate Improved WGAN-GP Sequences", key="comp_improved_miss_btn"):
                # Create a status container for messages
                status_container = st.empty()
                with st.spinner("Generating Improved WGAN-GP sequences..."):
                    try:
        ...
        if original_sequences and gumbel_sequences and improved_sequences:
            ...
            if 'Biological Significance' in metrics_df.index:
                ...
                st.warning("Biological significance information not available in the expected format.")
