...
# BioPython's GC function might be in a different location depending on the version
try:
    from Bio.SeqUtils import GC
except ImportError:
    # Define our own GC function
    def GC(seq):
        """Calculate GC content of a sequence."""
        seq = seq.upper()
        gc_count = seq.count('G') + seq.count('C')
        return 100.0 * gc_count / len(seq) if len(seq) > 0 else 0
