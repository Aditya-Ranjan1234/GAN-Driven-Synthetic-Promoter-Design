"""
Simplified script to generate DNA sequences using a Gumbel-Softmax GAN model.
...
def generate_random_dna_sequence(length=150, gc_bias=0.45):
    """
...
def generate_sequences(num_sequences, output_file, gc_bias=0.45, status_container=None):
    """
    ...
    with redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
        ...
        with open(output_file, 'w') as f:
            for i in range(num_sequences):
                ...
                f.write(f">gumbel_{i+1}\n")
                f.write(f"{seq}\n")

if __name__ == "__main__":
    # Suppress all output
    with redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
        parser = argparse.ArgumentParser(description="Generate DNA sequences using a simplified Gumbel-Softmax GAN model")
        ...
        parser.add_argument("--gc_bias", type=float, default=0.45,
