...
def parse_fasta(fasta_content):
    ...
    """
    # Create a StringIO object for SeqIO to parse
    ...
    for record in SeqIO.parse(fasta_io, "fasta"):
        ...
        sequence_id = id_parts[0] if id_parts else record.id
...
def main():
    ...
    for i, record in enumerate(records[:3]):
        print(f"Sequence {i+1}: {record['sequence_id']} - {record['sequence'][:50]}...")
